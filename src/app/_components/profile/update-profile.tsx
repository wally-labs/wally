...
export default function UpdateProfile({
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    // Default values should(?) be set by useEffect once data loads
  });
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button data-cy="update-profile-dialog-trigger" variant="outline">
          Edit Profile
        </Button>
      </DialogTrigger>
      <DialogContent
        data-cy="update-profile-dialog-content"
      ...
      >
        <ScrollArea>
          ...
          {isQueryLoading && (
            <div data-cy="update-profile-loading-query">Loading profile...</div>
          )}
          {isQueryError && (
            <div data-cy="update-profile-error-query">
              Error loading profile.
            </div>
          )}
          {!isQueryLoading &&
            !isQueryError &&
            (data ?? focusedChatData) && ( // Only render form if not loading/error and data is available
              <ProfileFormComponent
                form={form}
                handleSubmit={onSubmit}
                submitLabel="Update Profile"
                isPending={updateChatMutation.isPending}
                data-cy="profile-form-wrapper"
              />
            )}
        </ScrollArea>
      </DialogContent>
